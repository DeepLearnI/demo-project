This example was adapted from several different sources. 
We wanted to implement the lottery ticket model pruning technique,
so we forked from https://github.com/google-research/lottery-ticket-hypothesis

The README contents below is unmodified from the original source, but the actual
ML problem we're applying it to here is completely different
is completely different. Aside from its explanation of the Lottery Ticket 
hypothesis it may not be very useful.

...
from vanilla_tcn import VanillaTCNBlock
    ...
    def initialize(self, data, input_lengths, is_training=False, is_evaluating=True):
        ...
        with tf.variable_scope('inference'):
            ...
            if hp.recurrency_type == 'TCN':
                #Potential stack of vanilla TCN blocks
                tcn_blocks = []
                for i, units in enumerate(hp.tcn_units):
                    tcn_blocks.append(VanillaTCNBlock(units, hp.kernel_sizes[i], 1, dilation_rate=hp.dilations[i], dropout=hp.tcn_dropout[i], spatial_dropout=hp.spatial_dropout,
                        name='VanillaTCN_{}'.format(i)))

                for cell in tcn_blocks:
                    inputs = cell(inputs, training=is_training)
            ...
            #     tcn_blocks = []
            #     for i, units in enumerate(hp.tcn_units):
            #         tcn_blocks.append(BasicLSTMCell(units))
            #         # tcn_blocks.append(VanillaTCNBlock(units, hp.kernel_sizes[i], 1, dilation_rate=hp.dilations[i], dropout=hp.tcn_dropout[i], spatial_dropout=hp.spatial_dropout,
            #         #     name='VanillaTCN_{}'.format(i)))
            #     for cell in tcn_blocks:
            #         inputs = cell(inputs, training=is_training)
                ...
                while not self.coord.should_stop() and step < self.all_params.train_steps:
                    ...
                    if step % self.all_params.eval_interval == 0:
                        ...
                        for i in tqdm(range(4)):
                            val_loss, outputs, targets, lengths = sess.run([self.eval_model.loss, self.eval_model.outputs, self.eval_model.targets, self.eval_model.input_lengths])
                        ...
                        capture_rate, fig_path = evaluate_and_plot(outputs, targets, index=np.arange(0, len(targets)), model_name=self.all_params.name or self.all_params.self.model,
                                                                   weight=self.all_params.capture_weight, out_dir=self.eval_dir, use_tf=False, sess=sess, step=step)
